...
<div class="col-span-12 md:col-span-{{ $col }}">
    ...
    <textarea name="{{ $name }}" rows="{{ $rows }}" placeholder="{{ $placeholder ?: $label }}" {{ $attributes->merge(['class' => 'w-full px-4 py-3 bg-white border '.($hasError ? 'border-error' : 'border-outline-variant').' rounded-lg focus:border-primary-container focus:ring-1 focus:ring-primary-container outline-none transition-all font-body-sm resize-none']) }}>{{ $bound }}</textarea>
