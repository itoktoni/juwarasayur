<?php /** @var Modules\Inventory\Models\Gudang $table */ ?>
<x-layouts::app>
    <x-breadcrumb :items="[['url' => '/dashboard', 'label' => 'Home'], ['url' => '', 'label' => 'Warehouse']]" />
        ...
        <x-table>
            ...
            <x-slot:body>
                ...
                <tr>
                    ...
                    <td>{{ $table->gudang_kode ?? '-' }}</td>
                    <td>{{ $table->gudang_alamat ?? '-' }}</td>
            ...
            <x-slot:mobile>
                ...
                <x-table-mobile-list>
                    ...
                    <x-table-mobile-item :id="$table->field_primary">
                        <x-table-mobile-header :title="$table->gudang_nama" />
                        <div class="mt-2 space-y-1.5">
                            <div class="flex justify-between items-center gap-2">
                                <span class="text-xs text-on-surface-variant">Kode</span>
                                <span class="text-sm font-medium text-right">{{ $table->gudang_kode ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between items-center gap-2">
                                <span class="text-xs text-on-surface-variant">Status</span>
                                <span class="text-sm font-medium text-right"><x-badge :type="$table->is_active ? 'success' : 'default'">{{ $table->is_active ? 'Aktif' : 'Nonaktif' }}</x-badge></span>
                            </div>
                        </div>
