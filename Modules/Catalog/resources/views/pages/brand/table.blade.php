<?php /** @var Modules\Catalog\Models\Brand $table */ ?>

<x-layouts::app>
    <x-breadcrumb :items="[['url' => '/dashboard', 'label' => 'Home'], ['url' => '', 'label' => 'Brands']]" />
            ...
            <x-slot:mobile>
                ...
                <x-table-mobile-list>
                    ...
                    <x-table-mobile-item :id="$table->field_primary">
                        <x-table-mobile-header :title="$table->brand_nama" />
                        <div class="mt-2 space-y-1.5">
                            <div class="flex justify-between items-center gap-2">
                                <span class="text-xs text-on-surface-variant">Slug</span>
                                <span class="text-sm font-medium text-right">{{ $table->brand_slug }}</span>
                            </div>
                            <div class="flex justify-between items-center gap-2">
                                <span class="text-xs text-on-surface-variant">Status</span>
                                <span class="text-sm font-medium text-right">{{ $table->is_active ? 'Aktif' : 'Nonaktif' }}</span>
                            </div>
                        </div>
